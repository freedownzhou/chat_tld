...
export default {
  ...
  receiveRedPointMsg({state,commit},data){
    ...
    if (!localStorage.getItem('hongdian')) {
      redObj[data.sender] = [{'num':1,'message':data.message,'roomId':data.roomId}];
    } else {
      redObj = JSON.parse(localStorage.getItem('hongdian'));
      if (redObj[data.sender]) {
        redObj[data.sender][0].num++;
        redObj[data.sender] = [{'num':redObj[data.sender][0].num,'message':data.message,'roomId':data.roomId}];
  ...
  // },
    //roomId查询
  roomId_sql:({state,commit},roomId)=>{
    console.log(roomId);
    indexedDB.suo_room(state.chat.db,roomId);
  },
}
